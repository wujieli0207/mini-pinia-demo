      ...
      <van-tab :title="item.title" :name="item.name">
        ...
        <div>
          <van-field
            ...
            placeholder="请输入代办事项"
            right-icon="plus"
            @click-right-icon="todoStore.addTodo(newTodo)"
          />
        </div>

        <!-- 代办列表 -->
        <template v-for="item in todoStore.todos" :key="item.id">
          <van-cell :title="item.text" :value="item.text">
            <template #right-icon>
              <van-radio :name="item.isFinished" />
              <van-icon
                name="delete"
                @click="todoStore.removeTodoById(item.id)"
              />
            </template>
          </van-cell>
        </template>
